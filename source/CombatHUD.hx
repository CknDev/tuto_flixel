...
class CombatHUD extends FlxTypedGroup<FlxSprite>
{
	...
	public var e:Ennemy;		// we will pass the ennemy that the player touched to initialize combat, and this will let us also know which ennemy to kill, etc.
	...
	public var outcome(default, null):Outcome;	// when combat has finished, we will need to know if the player killed the ennemy or fled
	...
	private var _sprEnemy:Ennemy;	// this is a sprite of the ennemy

	// ** These variables will be used to track the ennemy's health
	private var _ennemyHealth:Int;
	private var _ennemyMaxHealth:Int;
	private var _ennemyHealthBar:FlxBar;	// This FlxBar will show us the ennemy's current/max health
	...
	public function new()
	{
		...
		// do the same thing for an ennemy. We'll just use ennemy type 0 for now and change it later.
		_sprEnemy = new Ennemy(_sprBack.x + 76, _sprBack.y + 16, 0);
		...
		// create and add a FlxBar to show the ennemy's health. We'll make it Red and Yellow.
		_ennemyHealthBar = new FlxBar(_sprEnemy.x - 6, _txtPlayerHealth.y, LEFT_TO_RIGHT, 20, 10);
		_ennemyHealthBar.createFilledBar(0xffdc143c, FlxColor.YELLOW, true, FlxColor.YELLOW);
		add(_ennemyHealthBar);
	 ...
	 */
	public function initCombat(PlayerHealth:Int, E:Ennemy):Void
	{
		...
		e = E;	// set our ennemy object to the one passed to us

		updatePlayerHealth();	// update the player health text

		// setup our ennemy
		_ennemyMaxHealth = _ennemyHealth = (e.etype + 1) * 2; // each ennemy will have health based on their type: Type 0 = 2 health, type 1 = 4 health
		_ennemyHealthBar.value = 100;	// the ennemy's health bar starts at 100%
		_sprEnemy.changeEnnemy(e.etype);	// change our ennemy's image to match their type.
	...
	private function makeChoice():Void
	{
		...
		switch (_selected)	// check which item was selected when the player picked it
		{
			case 0:
				...
				// ...the player attacks the ennemy first
				// they have an 85% chance to hit the ennemy
				if (FlxG.random.bool(85))
				{
					// if they hit, deal 1 damage to the ennemy, and setup our damage indicator
					_damages[1].text = "1";
					...
					_sndHurt.play();
					_ennemyHealth--;
					_ennemyHealthBar.value = (_ennemyHealth / _ennemyMaxHealth) * 100; // change the ennemy's health bar
				}
				else
				{
				...
				// position the damage text over the ennemy, and set it's alpha to 0 but it's visible to true (so that it gets draw called on it)
				_damages[1].x = _sprEnemy.x + 2 - (_damages[1].width / 2);
				_damages[1].y = _sprEnemy.y + 4 - (_damages[1].height / 2);
				_damages[1].alpha = 0;
				_damages[1].visible = true;

				// if the ennemy is still alive, it will swing back!
				if (_ennemyHealth > 0)
				{
					ennemyAttack();
				}
			...
			case 1:
				...
				{
					// if they fail to escape, the ennemy will get a free-swing
					ennemyAttack();
					FlxTween.num(_damages[0].y, _damages[0].y - 12, 1, { ease: FlxEase.circOut}, updateDamageY);
					FlxTween.num(0, 1, .2, { ease: FlxEase.circInOut, onComplete: doneDamageIn }, updateDamageAlpha);
				}
	...
	/**
	 * This function is called anytime we want the ennemy to swing at the player
	 */
	private function ennemyAttack():Void
	{
		...
		{
			// if the ennemy misses, show it on the screen
	 ...
	 * It will also check to see what we're supposed to do next - if the ennemy is dead, we trigger victory, if the player is dead we trigger defeat, otherwise we reset for the next round.
	 */
	private function doneDamageOut(_):Void
	{
		...
		else if (_ennemyHealth <= 0)
		{
			// if the ennemy's health is 0, we show the victory message
			outcome = VICTORY;
			_sndWin.play();
